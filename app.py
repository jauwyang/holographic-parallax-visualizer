...
class App:
    ...
    def main_loop(self):
        ...
        if file_type == "image":
            ...
            image_output_folder_path = os.path.join(
                "output", "images", filename_without_extension
            )
        elif file_type == "video":
            file_exists = input_video_exists(filename)
            image_output_folder_path = os.path.join(
                "output", "videos", filename_without_extension
            )
        ...
        if file_exists:
            ...
            if not os.path.exists(image_output_folder_path):
                ...
                if file_type == "image":
                    ...
                    layer_names = create_parallax_layers_images(
                        input_path, file_type, image_output_folder_path
                    )

                    write_image_metadata(filename, layer_names)

                elif file_type == "video":
                    video_all_frames_path = os.path.join(
                        image_output_folder_path, "frames"
                    )
                    ...
                    for i, frame in enumerate(video_frames):
                        ...
                        single_frame_path = os.path.join(
                            video_all_frames_path, frame_folder_name
                        )
                        os.makedirs(
                            single_frame_path
                        )  # creates single frame dir (e.g. './frame001/') <<< MAIN

                        os.makedirs(
                            os.path.join(single_frame_path, "depth_map")
                        )  # creates 'depth_map' dir for 1 frame (e.g. './frame001/depth_map/')
                        os.makedirs(
                        ...
                        # Create layers
                        layer_names = create_parallax_layers_images(
                            frame, file_type, single_frame_path
                        )
        ...
        if file_type == "image":
            ...
            parallax_image_layers = get_parallax_layers_image(
                image_output_folder_path, src_data["parallax_layers"]
            )

            width, height = (
                parallax_image_layers[0].shape[1],
                parallax_image_layers[0].shape[0],
            )
            ...
            for i in range(frame_count):
                counter = str(i).zfill(4)
                single_frame_path = os.path.join(
                    video_all_frames_path, "frame" + counter
                )
                parallax_video_frame_layers = get_parallax_layers_image(
                    single_frame_path, src_data["frames"]["parallax_layers" + counter]
                )

                pygame_frame_layers = []
                width, height = (
                    parallax_video_frame_layers[0].shape[1],
                    parallax_video_frame_layers[0].shape[0],
                )
